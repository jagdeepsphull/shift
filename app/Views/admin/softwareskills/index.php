                ...
                <table id="example1" class="table table-bordered table-striped" data-manual-order="1">
				  ...
				  if($shift_for){
					  foreach($shift_for as $index => $record){
						  ?>
						  <tr>
							<td><?php echo $record->ss_id;?></td>
							<td><?php echo $record->ss_name;?></td>
							<td><?php echo $status[$record->ss_status];?></td>
							<td><a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/edit/'.$record->ss_id);?>" class="btn btn-success">Edit</a> 
							<a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/delete/'.$record->ss_id);?>" class="btn btn-danger" onclick="return confirm('Are you sure? You want to delete')">Delete</a>
							<a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/changestatus/'.$record->ss_id);?>" class="btn btn-warning">Change Status</a>
							<?= view('partials/list_order_arrows', ['rows' => $shift_for, 'index' => $index, 'record' => $record, 'idKey' => 'ss_id', 'labelKey' => 'ss_name', 'link' => $pageinfo['link'], 'groupKey' => null]) ?>
							</td>
						  </tr>
						  <?php
					  }
