  ...
  <div class="card">
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped" data-manual-order="1">
                  ...
                  <?php 
				  if($province){
					  foreach($province as $index => $record){
						  ?>
						  <tr>
							<td><?php echo $record->p_id;?></td>
							<td><?php echo $record->p_name;?></td>
							<td><?php echo $status[$record->p_status];?></td>
							<td><a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/edit/'.$record->p_id);?>" class="btn btn-success">Edit</a> 
							...
							<a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/changestatus/'.$record->p_id);?>" class="btn btn-warning">Change Status</a>
							<?= view('partials/list_order_arrows', ['rows' => $province, 'index' => $index, 'record' => $record, 'idKey' => 'p_id', 'labelKey' => 'p_name', 'link' => $pageinfo['link'], 'groupKey' => null]) ?>
							</td>
						  </tr>
						  <?php
					  }
