-- ---------------------------------------------------------------------------
-- PickAShift - database update for the 2026-08-20 release
--
-- FIVE migrations, everything since `release-2026-08-14.sql`:
--
...
--   2026-08-20-090000  users.u_agreement_done     column
--
-- This SUPERSEDES `release-2026-08-19.sql`, which carried the first four. It is
-- that file with one section added, so a database already updated with it needs
-- only section 9 below - but running the whole thing again is fine; see "safe to
-- run twice".
--
-- The last one is what the 2026-08-20 release itself needs: the Agreement Done
-- tick box on the four back-office user forms - applicant add/edit and employer
-- add/edit - has nowhere to save without it, and those four screens fail on an
-- unknown column the moment somebody presses Save.
--
...
-- 9 -------------------------------------------- is the agreement on file? --
--
-- The Agreement Done tick box on the four back-office user forms: applicant
-- add/edit and employer add/edit. A yes or a no, so TINYINT rather than the INT
-- `u_status` beside it.
--
-- NO BACKFILL goes with this one, and that is deliberate. Nobody is asked the
-- question at registration and nothing outside `/sadmin` writes the column, so
-- 0 - "not yet" - is the truthful answer for every account that exists on the
-- day this runs. The office ticks them as the paperwork arrives.

SET @add_agreement := (
  SELECT IF(COUNT(*) > 0,
            'SELECT ''u_agreement_done already there'' AS note',
            'ALTER TABLE `users`
   ...
   WHERE `TABLE_SCHEMA` = DATABASE()
     AND `TABLE_NAME`   = 'users'
     AND `COLUMN_NAME`  = 'u_agreement_done'
);

PREPARE stmt FROM @add_agreement;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;


-- 10 --------------------------------------- the bookkeeping, for that one --

INSERT INTO `migrations` (`version`, `class`, `group`, `namespace`, `time`, `batch`)
SELECT
  '2026-08-20-090000',
  'App\\Database\\Migrations\\AddUserAgreementDone',
  'default',
  'App',
  UNIX_TIMESTAMP(),
  COALESCE((SELECT MAX(m.`batch`) FROM `migrations` m), 0) + 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM `migrations` m2 WHERE m2.`version` = '2026-08-20-090000'
);


-- 11 ------------------------------------------------------------ check it --
--
-- Every number below should be 1, and the last query should list all five
...
SELECT COUNT(*) AS u_agreement_done_present
  FROM `information_schema`.`COLUMNS`
 WHERE `TABLE_SCHEMA` = DATABASE()
   AND `TABLE_NAME`   = 'users'
   AND `COLUMN_NAME`  = 'u_agreement_done';
...
-- Nothing else to run.
--
-- What is in the 2026-08-20 release and needs no database change at all:
--
--   The Agreement Done tick box itself -> four views and four save paths in
--                                 Sadmin. The column in section 9 is the whole
--                                 of its schema. A clear box posts nothing, so
--                                 the controller writes 0 from the field being
--                                 absent rather than from its value - which is
--                                 what makes the tick undoable.
--
-- Everything carried over from the 2026-08-19 release - the pharmacist's
-- screens on the site theme, the hourly rate hidden from managers, the
-- read-only shift panel, the DataTables stylesheets, the self-naming Resources
-- and Contact pages - is code only and always was.
--
