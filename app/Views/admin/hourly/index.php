                ...
                <table id="example1" class="table table-bordered table-striped">
				  ...
				  if($hourly){
					  foreach($hourly as $record){
						  ?>
						  <tr>
							<td><?php echo $record->hr_id;?></td>
							<td><?php echo $record->hr_name;?></td>
							<td><?php echo $status[$record->hr_status];?></td>
							<td><a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/edit/'.$record->hr_id);?>" class="btn btn-success">Edit</a> 
							<a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/delete/'.$record->hr_id);?>" class="btn btn-danger" onclick="return confirm('Are you sure? You want to delete')">Delete</a>
							<a href="<?php echo base_url('sadmin/'.$pageinfo['link'].'/changestatus/'.$record->hr_id);?>" class="btn btn-warning">Change Status</a>
						  </tr>
						  <?php
					  }
				  }
